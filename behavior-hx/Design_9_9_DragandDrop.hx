...
class Design_9_9_DragandDrop extends ActorScript
{
			...
			if(wrapper.enabled)
			{
				...
				if(_Grabbed)
				{
					...
					if(isMouseReleased())
					{
						...
						Engine.engine.setGameAttribute("drag_in_progress", false);
					}
				}
				else
				{
					if((Engine.engine.getGameAttribute("drag_in_progress") == true))
					{
						return;
					}
					if(actor.isMousePressed())
					{
						...
						_Grabbed = true;
						propertyChanged("_Grabbed", _Grabbed);
						Engine.engine.setGameAttribute("drag_in_progress", true);
					}
				}
			}
		});
