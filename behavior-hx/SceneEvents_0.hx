...
class SceneEvents_0 extends SceneScript
{
	...
	public function new(dummy:Int, dummy2:Engine)
	{
		...
		_wanderpoint2 = 0.0;
		nameMap.set("num_bones", "_numbones");
		_numbones = 0;
	...
	override public function init()
	{
		...
		addWhenUpdatedListener(null, function(elapsedTime:Float, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled)
			{
				_numbones = asNumber(0);
				propertyChanged("_numbones", _numbones);
				for(actorOfType in getActorsOfType(getActorType(3)))
				{
					if(actorOfType != null && !actorOfType.dead && !actorOfType.recycled){
						_numbones = asNumber((_numbones + 1));
						propertyChanged("_numbones", _numbones);
					}
				}
		...
		addWhenDrawingListener(null, function(g:G, x:Float, y:Float, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled)
			{
				g.drawString("" + (("" + "Bones used:") + ("" + (("" + ("" + _numbones)) + ("" + "/3")))), 500, 20);
			}
		});
